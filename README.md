...
```
docs/index.html            랜딩 (문제 → 활동 → 살펴볼 곳 → 모집 지역 → 기대효과 → 데이터)
...
collect_gg.py              경기데이터드림 API 3종 수집 (키: 환경변수 GG_API_KEY)
...
## 처음 한 번만
1. 저장소 만들고 이 파일들 업로드 (Public 권장 — Pages 무료)
2. Settings › Secrets and variables › Actions › New repository secret: `GG_API_KEY` = 경기데이터드림 인증키
3. Settings › Pages › Deploy from a branch › main › /docs
4. Issues 탭 › Labels 에 `제보`, `확인됨`, `순찰일지` 라벨 생성
5. `docs/config.js` 의 repo 를 "계정명/저장소명" 으로 수정
6. Actions › "데이터 수집·제보 반영·배포" › Run workflow 1회
7. 접속: https://계정명.github.io/저장소명/
